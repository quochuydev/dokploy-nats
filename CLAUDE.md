...
- [x] WebSocket connection (`ws://` locally, `wss://` in Dokploy) — see `examples/node/views/index.html`
- [x] Request/reply with streamed events (`jobs.create` → `{ executionId }`, then `executions.<id>.>`) — see `examples/node/worker.ts` + `examples/node/views/index.html`
